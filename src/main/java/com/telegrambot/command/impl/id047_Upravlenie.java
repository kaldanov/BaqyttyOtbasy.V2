...
public class id047_Upravlenie extends Command {
...
//            bot.execute(new DeleteMessage(chatId, messagePrevReport));
        ...
        List<List<String>> info             = reportToServices.stream().map(x -> {
            ...
            String s = propertiesRepository.findById(Const.BOT_TOKEN).getValue();
            list.add("https://api.telegram.org/file/bot" + s + "/" + uploadFile(x.getFile()));
